...
def CreateOrgWebhook(org_id, webhook_url, auth_token, notify_type=None, notify_url=None, secret=None):
    api_url = "/api/v1/orgs/" + org_id + "/webhooks"
    request_url = "https://" + base_url + api_url
    print(request_url)
    header = {"content-type": "application/json", 'Authorization': 'token {}'.format(auth_token)}
    payload = {
    ...
    "verify_cert": False,
    "enabled": True,
    "topics": [ "alarms", "audits" ]}
    if (notify_type != None) and (notify_url != None):
        payload['headers'] = {
                "x-notify-type": notify_type.lower(),
                "x-notify-url": notify_url,
...
def DeleteOrgWebhook(org_id, webhook_id, auth_token):
    api_url = "/api/v1/orgs/" + org_id + "/webhooks/" + webhook_id
    request_url = "https://" + base_url + api_url
    print(request_url)
    header = {"content-type": "application/json", 'Authorization': 'token {}'.format(auth_token)}
    response = mist_Delete(api_url, auth_token)
